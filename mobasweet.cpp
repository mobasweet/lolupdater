...
const wchar_t* box[18] = {
	...
	L"Elder Scrolls Online", L"GameLoop (Android Emulator)", L"DirectX9", L"VCRedist AIO", L"Creative ALchemy", L"FinalBurn Neo", L"MAME", L"TrackMania Next", L"HBMAME"
};

static void UnblockFile(std::wstring file)
{
	DeleteFileW(file.append(L":Zone.Identifier").c_str());
}

static std::wstring PathJoin(const int j, const std::wstring& add)
{
	const std::filesystem::path p = n[j];
	return (p / add).c_str();
}

static void PathAppend(const int j, const std::wstring& add)
{
...
static void PathCombine(const int j, const int k, const std::wstring& add)
{
...
static void overwritefile(const int i, const int k, const std::wstring& add)
{
...
static void download(const std::wstring& url, int j)
{
...
static void bulk_apimswin(const wchar_t* url)
{
	for (int i = 0; i < 40; i++)
	{
...
static void cfg(const std::wstring& key)
{
	*n[0] = '\0';
	*n[82] = '\0';
	...
	if (std::wstring(n[0]).empty())
	{
		...
		if (key == L"ww")
		{
			i.lpszTitle = L"<drive>:\\Games\\World_of_Warships\\bin\\7******\\win64";
		}
		if (key == L"bd")
		{
...
static void game(const std::wstring& dir, const std::wstring& key)
{
	for (int i = 0; i < 57; i++)
	{
		*n[i] = '\0';
...
static void calchemy(bool restore)
{
...
static void dota2(bool restore)
{
...
static void lineage2(bool restore)
{
	for (auto i = 0; i < 8; i++)
	{
		*n[i] = '\0';
	}
	terminate_process(L"NCLauncherR.exe");
	terminate_process(L"NCLauncher.exe");
	terminate_process(L"L2.bin");
	cfg(L"l2");
	PathAppend(0, L"System");
	UnblockFile(PathJoin(0, L"L2.bin"));
	PathCombine(1, 0, L"d3dcompiler_43.dll");
	PathCombine(2, 0, L"d3dcompiler_47.dll");
	PathCombine(3, 0, L"D3DX9_40.dll");
	PathCombine(4, 0, L"vcomp120.dll");
	PathCombine(6, 0, L"plugins");
	if (restore)
	{
	...
	*n[0] = '\0';
	...
	sei.lpFile = PathJoin(0, L"NCWest\\NCLauncher\\NCLauncher.exe").c_str();
	ShellExecuteEx(&sei);
	if (sei.hProcess != nullptr)
	{
		WaitForSingleObject(sei.hProcess, INFINITE);
...
static void blackdesert(bool restore)
{
...
static void elderscrolls(bool restore)
{
	for (int i = 0; i < 3; i++)
	{
		*n[i] = '\0';
...
static void worldoftanks(bool restore)
{
	for (int i = 0; i < 46; i++)
	{
		*n[i] = '\0';
...
static void worldofwarships(bool restore)
{
	for (int i = 0; i < 46; i++)
	{
		*n[i] = '\0';
...
static void gameloop(bool restore)
{
...
static void leagueoflegends(bool restore)
{
	cfg(L"ll");
...
static void smite(bool restore)
{
...
static void paladins(bool restore)
{
...
static void directx9()
{
	*n[82] = '\0';
	PathAppend(82, std::filesystem::current_path());
	PathAppend(82, L"dxwebsetup.exe");
	download(L"dxwebsetup.exe", 82);
	sei = {};
	sei.cbSize = sizeof(SHELLEXECUTEINFOW);
	sei.fMask = 64;
	sei.nShow = 5;
...
static void winaio(bool restore)
{
	*n[0] = '\0';
...
static void trackmania(bool restore)
{
	...
	_In_ int nCmdShow)
{
	...
	LoadStringW(hInstance, IDC_MOBASWEET, szWindowClass, MAX_LOADSTRING);
	...
	HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_MOBASWEET));
	MSG msg;
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_MOBASWEET));
	wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_MOBASWEET);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	HWND hWnd = CreateWindow(szWindowClass, L"MOBASweet", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
		CW_USEDEFAULT, 470, 180,
		nullptr, nullptr, hInstance, nullptr);

	CreateWindow(L"BUTTON", L"Patch", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON, 10, 10, 100,
